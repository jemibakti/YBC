...
class Model_dop extends CI_Model{
   ...
   function get_table_wheres($nama_table,$where,$limit = null, $order=null, $short=null){
   // mendapatkan data tabel dengan suatu kondisi
        ...
        if($order){$this->db->order_by($order,$short);}
	...
	function tesSel2(){
			...
			$query = $this->db->select('id,nama as text')->get('m_treat');
			return $query->result();
	}
	
    function get_table_like($nama_table,$like,$nilai,$order=null,$short=null){
            // mendapatkan data tabel dengan suatu kondisi
            $this->db->like($like,$nilai);
            $this->db->order_by($order,$short);
            $query = $this->db->get($nama_table);
            return $query->result();
    }
